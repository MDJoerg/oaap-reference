...
check("der Mandantenspeicher bleibt fuer jeden Leser nur lesbar",
      ...
      # twin-secrets.json), seit 0.3 auch 'relay' (dieselben Zugaenge).
      # Frueher zaehlte diese Zeile die Leser (== 2) -- und schlug beim
      # dritten, ebenfalls nur lesenden Leser an, obwohl die Regel hielt.
      # Geprueft wird jetzt die Regel selbst: JEDER Mount dieser Ablage
      # ist ':ro', keiner beschreibbar -- egal, wie viele es werden.
      compose.count("/apps:/platform-apps") >= 3  # identity, twin, relay
      and compose.count("/apps:/platform-apps")
          == compose.count("/apps:/platform-apps:ro")
