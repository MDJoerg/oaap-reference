...
def write_external_caddy():
    """(Re)generate TLS sites for the registered external hostname.

    Portal on the apex, one subdomain per app instance — everything on
    443, certificates via Caddy's automatic ACME. Returns instance
    names skipped because their registry entry predates route capture.
    """
    host = load_external()
    ...
    if not host:
        ...
        return []
    reg = load_registry()
    lines = [f"# generated by appctl — registered external hostname: {host}"]
    ...
    for name, inst in sorted(reg["instances"].items()):
        ...
        if not routes or not inst.get("svc_port"):
            skipped.append(name)
            continue
        lines.append(f"https://{name}.{host} {{")
        lines += _LOG_BLOCK
...
def cmd_external(args):
    if args.action == "show":
        host = load_external()
        print(host if host else "No external hostname registered.")
        return
    ...
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"host": host}, f, indent=2)
